...
- `setup.sh` defaults to the `SidneyMe/livability-data` release `v1` mirror, which carries its five remote-install core inputs. Optional asset URLs are not guessed from that base because v1 does not carry them; configure their individual URLs when available. `DATASET_MIRROR_BASE_URL` can replace the core mirror, while per-dataset URLs still take precedence. (Confirmed)
...
- Transit reality is strictly GTFS-only. OSM stops are not imported or scored, and missing, broken, omitted, or `transit-unavailable` GTFS state yields zero transport signal rather than an OSM fallback. `precompute` refuses to publish a zero-transport build unless `--allow-missing-transport` is passed (gate lives in `precompute/workflow.py` right after `phase_amenities`).
