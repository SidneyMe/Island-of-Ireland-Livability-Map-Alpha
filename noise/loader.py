...
NOISE_CANDIDATES_CACHE_VERSION = 2
...
def normalize_ni_gridcode_band(gridcode: Any) -> tuple[float | None, float | None, str | None]:
    ...
    *,
    ...
    for member in _preferred_ni_entries(entries):
        ...
        if gdf.empty:
            continue
        for index, row in gdf.iterrows():
            db_low, db_high, db_value = normalize_ni_gridcode_band(row.get("gridcode") or row.get("GRIDCODE"))
            if db_value is None:
                continue
            geom = row.geometry
            if geom is None or geom.is_empty:
