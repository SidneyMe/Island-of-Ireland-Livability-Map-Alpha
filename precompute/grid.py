    ...
    *,
    ...
    for category, cap in CAPS.items():
        ...
        if effective_units is not None:
            effective_count = float(effective_units.get(category, 0.0))
        if category == "parks":
...
def _road_penalty_for_node(
    road_proximity_penalties_by_node,
    node: Any,
) -> float:
    if road_proximity_penalties_by_node is None:
        return 0.0
    node_index = int(node)
    try:
        if isinstance(road_proximity_penalties_by_node, dict):
            return max(float(road_proximity_penalties_by_node.get(node_index, 0.0)), 0.0)
        value = road_proximity_penalties_by_node[node_index]
    except (IndexError, KeyError, TypeError, ValueError):
        return 0.0
    try:
        return max(float(value), 0.0)
...
def score_cells(
    ...
    for index, (cell, node) in enumerate(zip(cells, cell_nodes)):
        ...
        effective_units = (
            {}
            if effective_units_by_node is None
            else {
        ...
        road_proximity_penalty = _road_penalty_for_node(
            road_proximity_penalties_by_node,
            node,
        )
        ...
        cell["effective_units"] = effective_units
        cell["scores"] = scores
        cell["total"] = total
